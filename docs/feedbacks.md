...
- [x] 在assemble 阶段，技术或实施方案中的三级目录应该有相应的编号和大纲级别 如 1. 就应还是H1 级别  1.1 应该是H2 级别， 1.1.1 应该是H3 级别，并将章节号附在标题前（2026-09-09 修订：run-20260908-215413 实证按锚降级方案失败——真实模板锚常在格式章深处 H3/H4，offset 降级致编号 1.1.1.1 爆炸、级别全钳 H4，且 scratch 产物 pStyle 在真实模板数字自编号 styleId 体系下解析不到塌回 Normal。最终设计：锚点只决定注入落点不再降级；number_headings 恒为 #→「1.」H1 / ##→「1.1」H2 / ###→「1.1.1」H3（同级递增、升级清零、围栏内行跳过）；大纲级别经 w:outlineLvl 直接写段落属性（0-based），不依赖 pStyle 能否在宿主包解析，导航窗格/目录照常识别；另修 replace_elements 空区间 no-op 致锚标题切片正文静默丢失——空区间改插锚标题后；真实 run 重生成验证 6/21/76 三级分布+编号正确；全量 245 passed）
