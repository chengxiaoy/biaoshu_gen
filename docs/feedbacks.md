...
- [x] build fill context 时把 requirements.yaml 的 purchase_list 注入 prompt（2026-09-11：plan 填写规则一直写着「按预注入的采购清单与 metadata 用 table op 填货物一览表」，但 build_fill_context 从未注入过采购清单——一览表填充只能靠模型猜。修复：metadata 段后新增【采购清单】段，yaml.safe_load 读 requirements.yaml 的 purchase_list 逐行注入，头部注明按此填一览表/供货范围、清单未给字段留空禁编造；真实 run 验证 3 条清单全部注入。测试 +1，全量 274 passed）
