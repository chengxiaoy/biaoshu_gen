...
# #89：模板的证照粘贴位是「xxx复印件」单列框表——图必须进框、禁止自创图注；
# 常见证照由代码预匹配锚点（picture_anchor_hints），agent 核对执行 + 兜长尾。
PICTURE_SYSTEM = (
...
- fill_skill.py：插图原语，直接 import 使用——
  · insert_picture_into_frame(行内文字关键词, 图片路径[, 宽度=4.8])：插进粘贴框
    表格的对应行内（证照粘贴位的**首选**方式）
  · insert_picture_after(段落前缀, 图片路径[, 宽度=5.6, 图注])：插在段落之后
    （无粘贴框的小节才用）

任务：为 {output} 插入应附的证照/证明图片（只做这一件事）：
1. **粘贴框**——单元格文字含「复印件/正反面/执照」的单列表格——是模板预留的
   证照粘贴位：图**必须插进框内对应行**（按行内文字定位），行内既有文字就是
   标签，**禁止再新增图注**；同一张证可按需插多个框（如法人身份证既要
   身份证明小节的框、也要授权委托书「法定代表人」行）
2. 无粘贴框的小节才用 insert_picture_after 插在小节标题/说明段之后，图注
   **只抄模板原文词**（如「营业执照」），禁止自创图注；图宽按版面 3.5~5.2 英寸
3. 依据 prompt 末尾的【插图预匹配清单】执行（代码按图片名↔文档锚点给出的
   确定性建议，先核对锚点文字存在再插）；标「无建议」的按文档实况判断
4. 与文档内容无关的图片跳过、不要硬插，但须在最终答复中列出并说明原因；
   禁止读取/查看图片内容

自查（插完逐项核对再收工）：
- 每个粘贴框行：要么图已在格内，要么 kb 确无对应证照（空框留人工）
- 全文无自创图注；每张图紧贴其标签行/小节
- kb 清单每张图：已插入（含多处）或在最终答复列明不适配原因
"""
