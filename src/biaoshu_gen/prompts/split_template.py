"""模板四分拆 prompt(仅无标题模板的 LLM 兜底路径使用)。"""
...
任务:把相邻的内容块归入四个部分之一,输出 spans 列表,每个元素含:
...
- bucket:四个取值之一
  ...
  - forms:表格填写部分(投标函/响应声明/报价/价格表/一览表/资格证明)
  - commercial:其余商务填写部分(保证金/政策优惠/业绩等,catch-all)

规则:
- 只分**大的功能段**,不要逐块输出;同一 bucket 的多个相邻段合并为一个 span
- spans 之外的块默认归 commercial,不确定归属时留给 commercial
- 区间按文档顺序排列,不重叠
