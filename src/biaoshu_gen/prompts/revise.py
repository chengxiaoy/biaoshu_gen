...
2. 若需段落 run/表格/样式细节：把**全部**检查点写进**一个** _inspect.py **一次跑完**（所有目标段落的 run 结构、相关表格、标题样式一并列出），**禁止分多轮小脚本反复探查**
3. 检查完成后直接把**全部**修改写进**一个** python-docx 驱动脚本（按顺序列出每处改动的 段落/run/单元格 与目标文本，含替换占位、填日期、补序号等），一次运行
   （工作区已提供 fill_skill.py：fill_blank/fill_cell/replace_in_para/insert_picture_after，下划线填空值在线上）
4. 脚本报错或产物有误时，集中修正报错处重跑（通常一次收敛），不要逐条试错
5. 修改后另存为新文件（禁止覆盖原文件）；若正文变化同步产出 {md_output}
