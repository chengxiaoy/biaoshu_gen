...
def split_template_node(state: BidState) -> dict:
    ...
    for bucket in BUCKETS:
        indexes = [i for i in assign.get(bucket, []) if i >= 0]
        if not indexes:
            continue                                      # 空 bucket 不落文件
        dest = parts_dir / PART_NAMES[bucket]
    ...
    (parts_dir / "parts.yaml").write_text(
        yaml.safe_dump({"order": order, "parts": manifest}, allow_unicode=True, sort_keys=False),
        encoding="utf-8")
