...
同桶多区间时附加段(如 (四)(五) 嵌在投标函/资格之间)由 run_with_extras
各跑一次独立工作区。
"""
from ..fill_context import SECTION_KEYWORDS, run_fill_node, run_with_extras
from ..prompts.commercial import SYSTEM, build_user_prompt
from ..state import BidState, run_dir


def _commercial_core(state: BidState, ws_key: str = "") -> dict:
    return run_fill_node(
        ...
        part="commercial", ws_key=ws_key,
    )


def commercial_node(state: BidState) -> dict:
    return run_with_extras(state, "commercial", _commercial_core)
